...
do_join(State, Ref, ChatName) -> %% TODO
	case maps:is_key(ChatName,State#cl_st.con_ch) of 
		%joining something you are in already -error
		true -> {err,State};
		% otherwise do the actual joining 
        false -> 
			whereis(server)!{self(),Ref,join, ChatName},
                receive
                    {ChatPID, Ref, connect, T} ->
					RevCons = maps:put(ChatName, ChatPID, State#cl_st.con_ch),
...
do_leave(State, Ref, ChatName) -> %% TODO
    case maps:find(ChatName, State#cl_st.con_ch) of
		%leaving something you aren't in -error
        error -> {err, State};
        {ok, _ChatPID} -> 
            ...
            receive 
				%update values
                {_From, Ref, ack_leave} -> 
					Temp = maps:remove(ChatName, State#cl_st.con_ch),
					NewState = #cl_st{
						gui = State#cl_st.gui,
						nick = State#cl_st.nick,
						con_ch = Temp
...
do_new_nick(State, Ref, NewNick) -> %% TODO
    case NewNick == State#cl_st.nick of
		% the new nickname is the curr/use
        true -> {err_same, State};
        false -> 
			whereis(server)!{self(), Ref, nick, NewNick},
            receive
				% check for last error case
...
do_msg_send(State, Ref, ChatName, Message) -> %% TODO
	%get the correlated room
   Room = maps:get(ChatName,State#cl_st.con_ch),
    Room ! {self(), Ref, message, Message},
	% does the actual sending of the messages
    receive
        {Room,Ref,ack_msg} -> {{msg_sent,State#cl_st.nick},State};
        _ -> {err,State}
    end.
